...
This paper's findings are crucial for policy makers, social service organizations, and urban planners. By shedding light on the trends in shelter occupancy and capacity, the study identifies gaps in service and potential areas for improvement. The analysis has implications for future resource allocation, as understanding the patterns of shelter usage can help design better support strategies, optimize capacity, and improve the overall response to homelessness in urban areas. Additionally, it can serve as a model for analyzing shelter systems in other cities, offering a framework for data-driven decision-making.

The remainder of this paper is structured as follows. Section 2(@sec-data) provides details on the data source, including the specific variables used, and describes the data cleaning process to prepare for analysis. This data section also outlines the tools and methodologies applied. Section 3(@sec-results) presents the key results, focusing on occupancy rates, service user counts, and actual capacity over time. Visualizations are used to intuitively convey these findings. Section 4{@sec-dis} offers a discussion of the results and their implications, summarizing conclusions and emphasizing the importance of balancing shelter occupancy and capacity for effective service provision. The comprehensive structure ensures that the reader gains a full understanding of what was analyzed, the findings, and their significance.
...
## Data Tools
The data was simulated, extracted, and cleaned by R[@citeR] from the opendatatoronto package[@citeopendatatoronto]. Tidyverse[@citetidyverse], here[@citehere], ggplot2[@citeggplot2], dplyr[@citedplyr], lubridate[@citelubridate], janitor[@citejanitor] and knitr[@citeknitr] are all applied in the analysis.
...
# Result{#sec-results}
Data visualizations can be used to present results more intuitively, and the data used to visualize  is from @citeopendatatoronto. 
...
Figure 2(@fig-year-count) shows the number of service users across different years from 2021 to 2024, as also detailed in Table 3(@tbl-total). The graph highlights a clear upward trend from 2021 to 2023, followed by a decline in 2024. However, the total number of service users in 2024 remains higher than in 2021.
...
df_long <- tidyr::pivot_longer(avg_rate, cols = c("avg_rate_bed", "avg_rate_room"),
                               names_to = "type",
                               values_to = "average")
...
# Reshape the data to long format for plotting with side-by-side bars
df_long <- tidyr::pivot_longer(total_cap, cols = c("total_cap_bed", "total_cap_room"),
                               names_to = "type",
                               values_to = "total")
...
Figure 4(@fig-total-cap) illustrates the total actual capacity of beds and rooms from 2021 to 2024, allowing for a comparison of the capacities across two categories. Notably, as shown in Figure 3(@fig-rate-mean), rooms consistently exhibit a higher occupancy rate than beds over all four years. The plots indicate that the total actual capacity of rooms was only greater than that of beds in 2021 and 2022; however, in 2023 and 2024, the capacity of beds surpassed that of rooms significantly. Additionally, the total actual capacity of beds demonstrated an increasing trend from 2021 to 2023, followed by a decline in 2024. Conversely, the total actual capacity of rooms exhibited an upward trend from 2021 to 2022, after which it began to decrease until reaching its lowest point in 2024.
